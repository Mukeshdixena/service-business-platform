...
public class BookingService {
    ...
    public BookingDto create(UUID businessId, UUID customerUserId, CreateBookingRequest request) {
        ...
        if (service.getBookingType() != ServiceBookingType.APPOINTMENT) {
            throw new ValidationException("serviceId: only APPOINTMENT services support booking in this phase.");
        }
        ...
        Instant endAt = startAt.plusSeconds(service.getDurationMinutes() * 60L);
        requireWithinBusinessHours(businessId, startAt, endAt);

        acquireSlotLock(businessId, staffId);
        requireNoOverlap(businessId, staffId, startAt, endAt);
        ...
        Booking booking = new Booking(businessId, customerProfileId, service.getId(), staffId, startAt, endAt,
                service.getPrice(), service.getCurrency(), request.notes());
    ...
    private void requireNoOverlap(UUID businessId, UUID staffId, Instant startAt, Instant endAt) {
        ...
        boolean conflict = candidates.stream()
                .filter(b -> Objects.equals(b.getStaffId(), staffId))
    ...
    /**
     * Serializes concurrent booking attempts for the same (business, staff)
     * pair. A Postgres transaction-scoped advisory lock is used instead of
     ...
     */
    private void acquireSlotLock(UUID businessId, UUID staffId) {
        long key = lockKey(businessId, staffId);
        entityManager.createNativeQuery("select pg_advisory_xact_lock(:key)")
                .setParameter("key", key)
                .getSingleResult();
    }

    private long lockKey(UUID businessId, UUID staffId) {
        int hash = Objects.hash(businessId, staffId);
        return hash;
    }
    ...
    public static BookingDto toDto(Booking b) {
        return new BookingDto(b.getId(), b.getBusinessId(), b.getCustomerId(), b.getServiceId(), b.getStaffId(),
                b.getStartAt(), b.getEndAt(), b.getStatus().name(), b.getPrice(), b.getCurrency(), b.getNotes(),
